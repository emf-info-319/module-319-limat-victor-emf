...
public class ExercicesMethodesTableaux {
    ...
    public static void main(String[] args) {
        ...
        double moyenneValeursTab = retournerMoyenneValeursTab(newTab);
        int valeurAChercherTabRemplacer = 4;
        int valeurDeRemplacement = 9;
        int[] valeursTabRemplacées = retournerTabValeursRemplacées(newTab, valeurAChercherTabRemplacer,
                valeurDeRemplacement);
        System.out.println("Après remplacement de la valeur " + valeurAChercherTabRemplacer + " par la valeur "
                + valeurDeRemplacement + " voici le contenu du tableau : " + valeursTabRemplacées);
        ...
        int dernièrePositionValeurRecherchée = retournerPositionCelluleDernièreFois(newTab,
                valeurAChercherDernièrePosition);
    ...
    public static int[] remplirTabValeurFixe(int[] tab, int valeur) {
    ...
    public static int[] remplirTab(int[] tab, int maxValeur) {
        for (int i = 0; i < tab.length; i++) {
            tab[i] = (int)(Math.random() * (maxValeur - 0 + 1)) + 0;
        }
        return tab;
    }
    ...
    public static void afficherContenuTab(int[] tab) {
        for (int i = 0; i < tab.length; i++) {
    ...
    public static double retournerMoyenneValeursTab(int[] tab) {
        int totalValeurs = 0;
        for (int i = 0; i < tab.length; i++) {
            totalValeurs += tab[i];
        }
        double moyenneValeurs = (int) totalValeurs / tab.length;
